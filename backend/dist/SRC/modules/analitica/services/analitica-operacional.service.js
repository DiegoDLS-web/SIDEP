...
const getAnaliticaOperacionalReporte = async (anioParam, mesParam) => {
    ...
    const unidadesMes = await prisma_1.default.unidadEnEmergencia.findMany({
        where: {
    ...
    const unidadesMesAnt = await prisma_1.default.unidadEnEmergencia.findMany({
        where: {
            parte: (0, partes_where_1.parteWhereNoAnulado)({
    ...
    const checklistEjecuciones = await prisma_1.default.checklistEjecucion.findMany({
        where: {
    ...
    const asistenciasAnio = await prisma_1.default.asistenciaPersonal.findMany({
        where: {
            parte: (0, partes_where_1.parteWhereNoAnulado)({
    ...
    const partesConMetaAsistencia = await prisma_1.default.parteEmergencia.findMany({
    ...
    for (const parte of partesConMetaAsistencia) {
        let meta = null;
        try {
            meta = parte.metadata ? JSON.parse(parte.metadata) : null;
        }
        catch {
            meta = null;
        }
        ...
        for (const ctx of Object.values(apc)) {
            for (const [id, mark] of Object.entries(ctx || {})) {
                ...
                if (!rut || rutsConFila.has(`${parte.id}:${rut}`))
                    continue;
    ...
    const partesMes = await prisma_1.default.parteEmergencia.findMany({
        where: (0, partes_where_1.parteWhereNoAnulado)({
            fechaEmergencia: { gte: inicioMes, lte: finMes },
        }),
        include: { clave: true },
    ...
    const asistenciaVoluntariosDetallePorMes = Array.from({ length: 12 }, (_, i) => {
        ...
        const voluntarios = Object.entries(userMap)
            .filter(([, data]) => {
            if ((data.rol ?? '').trim().toUpperCase() === 'ADMIN')
                return false;
