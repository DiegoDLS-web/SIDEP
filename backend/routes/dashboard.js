...
exports.dashboardRouter.get('/resumen', async (req, res) => {
    ...
    try {
        ...
        const rowsUnidad = await prisma_js_1.prisma.checklistCarro.findMany({
            where: { tipo: 'UNIDAD' },
            orderBy: { fecha: 'desc' },
            select: {
                carroId: true,
        ...
        const ultimosCheckUnidad = rowsUnidad.filter((r) => {
            if (seenU.has(r.carroId))
                return false;
            seenU.add(r.carroId);
            return true;
        });
        const rowsEra = await prisma_js_1.prisma.checklistCarro.findMany({
            where: { tipo: 'ERA' },
            orderBy: { fecha: 'desc' },
            select: {
                carroId: true,
        ...
        const ultimosCheckEra = rowsEra.filter((r) => {
            if (seenE.has(r.carroId))
                return false;
            seenE.add(r.carroId);
            return true;
        });
        const rowsTrauma = await prisma_js_1.prisma.checklistCarro.findMany({
            where: { tipo: 'TRAUMA' },
            orderBy: { fecha: 'desc' },
            select: {
                carroId: true,
        ...
        const now = new Date();
        const mesParaStat = anio === now.getFullYear() ? now.getMonth() + 1 : 12;
