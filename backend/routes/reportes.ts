...
function esTipoExcluidoAsistencia(tipo: string | null | undefined): boolean {
  const t = String(tipo ?? '')
    .trim()
    .toUpperCase();
...
function wherePartesSinDemo(fecha: { gte: Date; lte: Date }) {
  return {
    fecha,
    NOT: [
      { direccion: { contains: 'demo', mode: 'insensitive' as const } },
      { direccion: { contains: 'prueba', mode: 'insensitive' as const } },
    ],
  };
}
...
function diffMin(inicio: Date | null, fin: Date | null): number | null {
...
function extraerSector(direccion: string): string {
...
function extraerAsistenciasMetadata(metadata: unknown): Array<{ usuarioId: number; presente: boolean }> {
...
reportesRouter.get('/analitica-operacional', async (req, res) => {
  const hoy = new Date();
  ...
  try {
    ...
    const idsUsuariosAsistenciaValidos = new Set<number>(usuariosAsistenciaValidos.map((u) => u.id));

    const tiemposDespacho: number[] = [];
    const tiemposRespuesta: number[] = [];
    const duraciones: number[] = [];
    const sectoresMap = new Map<string, { totalMin: number; casos: number }>();
    const usoUnidadMap = new Map<number, { id: number; nomenclatura: string; salidas: number; km: number }>();
    const checksPorUnidadMap = new Map<number, Set<string>>();
    ...
    for (const chk of checklists) {
      ...
      if (!checksPorUnidadMap.has(chk.carroId)) checksPorUnidadMap.set(chk.carroId, new Set());
    ...
    for (const p of partes) {
      ...
      for (const u of p.unidades) {
        ...
        if (duracion !== null) duraciones.push(duracion);
    ...
    const promedio = (vals: number[]): number =>
      vals.length ? Number((vals.reduce((a, b) => a + b, 0) / vals.length).toFixed(2)) : 0;
    ...
    const cumplimientoRespuesta8MinPct =
      tiemposRespuesta.length > 0 ? Number(((respuestasEnSla / tiemposRespuesta.length) * 100).toFixed(2)) : 0;
    ...
    for (const p of partesMesAnterior) {
      ...
      for (const u of p.unidades) {
        const deltaKm = Number(u.kmLlegada) - Number(u.kmSalida);
        if (Number.isFinite(deltaKm) && deltaKm >= 0) kilometrosMesAnterior += deltaKm;
      }
    }
    const variacionPct = (actual: number, anterior: number): number => {
    ...
    const salidasPorSemanaMap = new Map<number, number>();
    for (let s = 1; s <= semanasMes; s++) salidasPorSemanaMap.set(s, 0);
    for (const p of partes) {
      const d = new Date(p.fecha);
    ...
    const asistenciaPorMes = new Map<number, { presentesUnicos: Set<number>; asistenciasRegistradas: number }>();
    const asistenciaDetallePorMes = new Map<
      number,
      Map<number, { asistencias: number; partes: Set<number> }>
    >();
    for (let idx = 1; idx <= 12; idx++) {
      asistenciaPorMes.set(idx, { presentesUnicos: new Set<number>(), asistenciasRegistradas: 0 });
      asistenciaDetallePorMes.set(idx, new Map<number, { asistencias: number; partes: Set<number> }>());
    }
    for (const parte of partesAnio) {
      ...
      for (const item of asistencias) {
        if (!item.presente) continue;
        if (!idsUsuariosAsistenciaValidos.has(item.usuarioId)) continue;
        bucket.presentesUnicos.add(item.usuarioId);
        bucket.asistenciasRegistradas += 1;
        if (detalleMes) {
          const curr = detalleMes.get(item.usuarioId) ?? { asistencias: 0, partes: new Set<number>() };
          curr.asistencias += 1;
    ...
    const asistenciaVoluntariosTotalAnual = asistenciaVoluntariosPorMes.reduce(
      (acc, row) => acc + row.asistenciasRegistradas,
      0,
    );
    const nombreUsuarioPorId = new Map(
      usuariosAsistenciaValidos.map((u) => [u.id, { nombre: u.nombre, rol: u.rol, cargo: u.cargoOficialidad ?? null }]),
    );
...
reportesRouter.get('/cuadro-honor', async (req, res) => {
  const hoy = new Date();
  const anio = Number(req.query.anio) || hoy.getFullYear();
  const mes = Number(req.query.mes) || hoy.getMonth() + 1;
  if (!Number.isFinite(anio) || !Number.isFinite(mes) || mes < 1 || mes > 12) {
  ...
  try {
    ...
    const diasMes = new Map<number, Set<string>>();
    const diasAnio = new Map<number, Set<string>>();
    const diasQuincena = new Map<number, Set<string>>();
    const idsUsuariosValidos = new Set<number>(usuariosValidos.map((u) => u.id));

    const acumular = (rows: Array<{ fecha: Date; metadata: unknown }>, target: Map<number, Set<string>>) => {
      for (const row of rows) {
        const meta = row.metadata as
          ...
          | null;
        const apc = meta?.asistencia?.asistenciaPorContexto;
        if (!apc || typeof apc !== 'object') continue;
        const dia = row.fecha.toISOString().slice(0, 10);
        for (const mapa of Object.values(apc)) {
          if (!mapa || typeof mapa !== 'object') continue;
          for (const [k, v] of Object.entries(mapa)) {
            if (!v || !k.startsWith('usr-')) continue;
            const uid = Number(k.slice(4));
            if (!Number.isFinite(uid) || uid <= 0) continue;
            if (!idsUsuariosValidos.has(uid)) continue;
    ...
    acumular(
      partesMes.filter((r) => new Date(r.fecha).getTime() <= finQuincena.getTime()),
      diasQuincena,
    );
