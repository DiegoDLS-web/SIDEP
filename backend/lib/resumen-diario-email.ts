...
async function tickResumenDiario(): Promise<void> {
  ...
  const listaHtml =
    total > 0
      ? `<ul style="padding-left:18px">${rows
          ...
          .map(
            (r) =>
              `<li><b>${escapeHtml(r.correlativo)}</b> — ${escapeHtml(r.claveEmergencia)} — ${escapeHtml(r.estado)}<br/><span style="color:#555">${escapeHtml(r.direccion)}</span></li>`,
          )
